...
class Testdatabase_obj:
    ...
    def test_execute_insert(self):
        ...
        assert self.data.execute_insert('test_tb_2',
                                        columnnames=['id', 'time', 'time_null', 'description', 'description_null'],
                                        attributes=[(id, timestamp, None, 'test_execute_insert', None)]) == 1
    ...
    def test_insert_update(self):
        ...
        assert self.data.insert_update(
            'INSERT INTO test_tb_0 (title, gender, description, phone, statue) VALUES (%s,%s,%s,%s,%s)',
            [('test23', '1', 'test_insert_update', '87123212', '1')]) == 1

    def test_execute_row_affected(self):

        assert self.data.execute_row_affected(
            "INSERT INTO test_tb_0 (title, gender, description, phone, statue) VALUES ('test23','1','test_execute_row_affected','87123212','1')") == 1

    def test_select_from_table(self):

        self.data.execute_insert('test_tb_0', columnnames=['title', 'gender', 'description', 'phone', 'statue'],
                                 attributes=[('test', '1', 'test_select_from_table', '874562', '1')])
    ...
    def test_execute_delete(self):
        ...
        self.data.execute_insert('test_tb_0', columnnames=['title', 'gender', 'description', 'phone', 'statue'],
                                 attributes=[('test', '1', 'test_execute_delete', '874562', '1')])
    ...
    def test_execute_select(self):
        ...
        self.data.execute_insert('test_tb_0', columnnames=['title', 'gender', 'description', 'phone', 'statue'],
                                 attributes=[('test', '1', 'test_execute_select', '874562', '1')])
        self.data.execute_update('test_tb_0', 'id = 1',
                                 columnnames=['description'],
                                 attributes=[(description,)])
        self.data.execute_insert('test_tb_1', columnnames=['id', 'second_id', 'title'],
                                 attributes=[('1', id, 'test_execute_select')])
        result = self.data.execute_select(
            'SELECT test_tb_0.description, test_tb_1.second_id, test_tb_1.title FROM test_tb_0 INNER JOIN test_tb_1 ON test_tb_0.id = test_tb_1.id')
        flag = False
    ...
    def test_read_access_account(self):
        ...
        assert not read.execute_update('test_tb_2', 'id = {}'.format(id),
                                       columnnames=['description_null'],
                                       attributes=[(str(ts),)])
        assert not read.insert_update(
            'INSERT INTO test_tb_0 (title, gender, description, phone, statue) VALUES (%s,%s,%s,%s,%s)',
            [('test23', '1', 'test_insert_update', '87123212', '1')])
        assert not read.execute_row_affected(
            "INSERT INTO test_tb_0 (title, gender, description, phone, statue) VALUES ('test23','1','test_execute_row_affected','87123212','1')")
        ...
        self.data.execute_update('test_tb_0', 'id = 1',
                                 ...
                                 attributes=[(description,)])
        self.data.execute_insert('test_tb_1', columnnames=['id', 'second_id', 'title'],
                                 attributes=[('1', id, 'test_read_access_account')])

        result = read.execute_select(
            'SELECT test_tb_0.description, test_tb_1.second_id, test_tb_1.title FROM test_tb_0 INNER JOIN test_tb_1 ON test_tb_0.id = test_tb_1.id')
        print(result)
    ...
    def test_execute_insert_update(self):
        ts = time.time()
        timestamp = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        id = str(int(ts / 86400) % 365) + str(int(ts) % 86400)
        assert self.data.execute_insert_update('test_tb_2',
                                        columnnames=['id', 'time', 'time_null', 'description', 'description_null'],
                                        attributes=[(id, timestamp, None, 'test_execute_insert', None)],
                                               to_update=['time=VALUES(time)', 'time_null=VALUES(time_null)', 'description=VALUES(description)', 'description_null=VALUES(description)']) == 1
